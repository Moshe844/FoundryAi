...
function latestNarrative(timeline: FactoryExecutionEvent[]) {
  return timeline
    .filter((event) => !isInternalExecutionEvent(event) && (event.tier === "finding" || event.tier === "decision" || event.tier === "flag" || event.kind === "reasoning"))
...
export function MissionFocus({ mission, request }: { mission: ExecutionMission; request: string }) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-overlay/10 bg-[linear-gradient(145deg,rgba(28,37,38,.9),rgba(11,15,16,.95))] shadow-[0_18px_60px_rgba(0,0,0,.2)]">
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-overlay/10 px-4 py-3.5 sm:px-5">
        <div className="min-w-0">
          ...
          {expanded && clean(request) !== clean(heading) ? (
            <details className="mt-2 max-w-3xl">
              ...
              <p className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap rounded-md border border-overlay/8 bg-shade/20 p-3 text-xs font-normal leading-5 text-foundry-muted">{request}</p>
            </details>
          ) : null}
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button type="button" onClick={() => setExpanded((value) => !value)} className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-overlay/10 text-foundry-subtle transition hover:border-overlay/25 hover:text-foundry-ink" aria-label={expanded ? "Collapse active mission" : "Expand active mission"} aria-expanded={expanded}>
      ...
      {expanded ? <div className="grid gap-4 px-4 py-4 sm:px-5">
        ...
        {plan.length > 3 ? (
          <div className="grid gap-2 border-t border-overlay/8 pt-3 sm:grid-cols-2">
            <div>
        ...
        {mission.state === "verifying" || mission.state === "complete" ? (
          <div className="flex items-start gap-2 border-t border-overlay/8 pt-3 text-xs leading-5 text-foundry-muted">
