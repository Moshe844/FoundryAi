...
export const DISCOVERY_REFINEMENT_SYSTEM_PROMPT = [
  ...
  "stack_options: propose 3-5 concrete stacks that could ALL build the exact same architecture/features/data-model you just decided — i.e. language/framework variants of one plan, not independent guesses at different products. Span genuinely different languages where Foundry has meaningful support (at least one JS/TS option, one Python option, and where relevant a JVM/.NET/Go option), each named specifically (e.g. 'Next.js (TypeScript)', 'Python/FastAPI', 'Java/Spring Boot' — not just 'JavaScript'). Exactly one entry has recommended:true — your actual best pick, matching recommended_stack. Each 'why' is one short sentence on why that specific stack fits THIS project, not a generic language pitch.",
