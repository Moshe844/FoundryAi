...
export function createExecutionStrategy(signals: MissionSignals): ExecutionStrategy {
  ...
  if (signals.highRisk || signals.complexity === "critical") {
    return strategy("staged-migration", 1, "High-risk work needs ordered checkpoints, compatibility review, and rollback-aware verification.", [assignment("discover", "builder", "Inventory current behavior before changing architecture."), assignment("plan", "architect", "Architecture-scale sequencing needs strong cross-system reasoning."), assignment("review", "architect", "Review risks before implementation."), assignment("implement", "architect", "Changes are coupled and should remain staged."), assignment("debug", "architect", "Diagnosing a failure in coupled architecture-scale work needs strong reasoning."), assignment("verify", "builder", "Evaluate the full evidence set."), assignment("repair", "architect", "Escalate unresolved failures.")]);
  }
  ...
  if (signals.complexity === "trivial" || (signals.complexity === "small" && signals.fileCount <= 20)) {
    return strategy("focused-edit", 1, "The task is localized and should avoid project-wide ceremony.", [assignment("discover", "fast", "Locate only the relevant files."), assignment("implement", "fast", "Apply the bounded low-risk change without buying unnecessary intelligence."), assignment("debug", signals.repeatedFailures ? "architect" : "builder", signals.repeatedFailures ? "A repeated failure in localized work is no longer a simple defect." : "A localized defect is ordinary diagnosis work."), ...(signals.needsVisualValidation ? [assignment("visual", "fast", "Validate the affected UI directly.")] : []), assignment("verify", "fast", "Run the smallest relevant check."), assignment("repair", signals.repeatedFailures ? "architect" : "builder", signals.repeatedFailures ? "A repeated failure needs stronger diagnosis." : "Escalate only if verification exposes a real repair need.")]);
  }
